...
fn insert(x : u64, t : BSTree) -> BSTree {
    match t {
	BSTree::Empty =>
	    BSTree::Node { val: x,
			   lc: 0, left: Box::new(BSTree::Empty),
			   rc: 0, right: Box::new(BSTree::Empty)
	    },
	BSTree::Node { val: v, lc, left: l, rc, right: r } => {
	    if x < v {
		let l_ = insert(x, *l);
		BSTree::Node { val: v, lc: lc + 1, left: Box::new(l_), rc: rc, right: r }
	    } else {
		let r_ = insert(x, *r);
		BSTree::Node { val: v, lc: lc, left: l, rc: rc + 1, right: Box::new(r_) }
	    }
	}
    }
}

fn nth(n : i64, t : BSTree) -> i64 {
    match t {
	BSTree::Empty => -1,
	BSTree::Node { val: v, lc, left: l, rc: _rc, right: r } => {
	    if n < lc {
		nth(n, *l)
	    } else if n == lc {
		v as i64
