...
* Write a program that takes a filename as a command-line argument and parses it into a DOM tree using an appropriate XML library:
  * Go: xmldom
  * Julia: LightXML
  * OCaml: markup
  * Rust: minidom
* Make sure your program:
  * Exits normally without error when given a valid XML file
  * Raises an error when given a non-existent file
* The behavior for a file containing invalid XML is left to the library (some raise errors, others silently fix or skip broken parts)
* Define a function `readxml` that takes a filename and returns a DOM tree, and a `main` function that calls it
