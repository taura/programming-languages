...
* Write a function `add_or_mul` __in assembly__, using a conditional select (`csel`) instead of a conditional branch.
* That is, translate the following C function into assembly:
```
long add_or_mul(long x, long y, long z) {
...
* Fill in the skeleton `add_or_mul.s` (after `// ------- write your answer here -------`).
* The checker `check_add_or_mul.c` verifies your result. If you see `OK`s and no errors, you are done.
