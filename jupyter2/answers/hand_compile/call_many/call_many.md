...
* Write a function `call_many(base)` __in assembly__ that calls the (given) twenty-parameter function `add_many` with the arguments `base, base+1, ..., base+19`, and returns its result.
...
* Fill in the skeleton `call_many.s` (after `// ------- write your answer here -------`).
* The checker `check_call_many.c` provides `add_many` and verifies your result. If you see `OK`s and no errors, you are done.
