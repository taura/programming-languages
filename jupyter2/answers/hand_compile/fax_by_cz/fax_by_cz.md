...
* Write a function `fax_by_cz` that returns `a*x + b*y + c*z` for given `double` inputs `a, x, b, y, c, z`, __in assembly__.
* This is the floating-point counterpart of the `ax_by_cz` problem: the same expression, but with `double` parameters and results.
* That is, translate the following C function into assembly:
```
double fax_by_cz(double a, double x, double b, double y, double c, double z) {
  return a * x + b * y + c * z;
}
```
* Fill in the skeleton `fax_by_cz.s` (after `// ------- write your answer here -------`) with instructions.
* The checker `check_fax_by_cz.c` compares your result against the value computed in C (within a small tolerance). If you see `OK`s and no errors, you are done.
