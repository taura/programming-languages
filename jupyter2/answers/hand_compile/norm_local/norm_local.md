...
* Write a function `norm_local(a, b, c)` __in assembly__ that creates a local (stack) array of three `long`s, stores `a, b, c` into it, and returns `l2_norm_long` of it.
* That is, translate the following C function into assembly:
```
long l2_norm_long(long * x);   /* given; computes x[0]*x[0]+x[1]*x[1]+x[2]*x[2] */

long norm_local(long a, long b, long c) {
  long x[3];
  x[0] = a; x[1] = b; x[2] = c;
...
* Fill in the skeleton `norm_local.s` (after `// ------- write your answer here -------`).
* The checker `check_norm_local.c` provides `l2_norm_long` and verifies your result. If you see `OK`s and no errors, you are done.
