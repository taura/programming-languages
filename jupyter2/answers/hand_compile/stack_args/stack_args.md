...
* Write a function `stack_args` that takes **twenty** `long` parameters but uses only two of them: it returns `a08 * 1000 + a15`, __in assembly__.
* That is, translate the following C function into assembly:
```
long stack_args(long a00, long a01, long a02, long a03, long a04,
...
* Fill in the skeleton `stack_args.s` (after `// ------- write your answer here -------`).
* The checker `check_stack_args.c` verifies your result. If you see `OK`s and no errors, you are done.
