...
* Write a function `sum_array(a, n)` that computes the sum of an `n`-element array of `long`s `a`, __in assembly__.
...
```
* Fill in the skeleton `sum_array.s` (after `// ------- write your answer here -------`).
* The checker `check_sum_array.c` verifies your result. If you see `OK`s and no errors, you are done.
