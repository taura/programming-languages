...
* Write a function `ax_by_cz` that returns `a*x + b*y + c*z` for given `long` inputs `a, x, b, y, c, z`, __in assembly__.
...
```
* Fill in the skeleton `ax_by_cz.s` (after `// ------- write your answer here -------`) with instructions.
* The checker `check_ax_by_cz.c` compares your result against the value computed in C. If you see `OK`s and no errors, you are done.
