...
%%%%%%%%%%%%%%%%% 
\begin{frame}
  \frametitle{Assignments do {\it not copy}, but {\it move}, the reference}
  \begin{itemize}
  \item to maintain ``single-owner'' rule,
    an assignment in Rust {\it moves} the value out of the righthand side,
...
%%%%%%%%%%%%%%%%% 
\begin{frame}
  \frametitle{Note: exceptions to ``assignment moves the reference''}
  \begin{itemize}
...
\begin{frame}
  ...
  \item making a pointer by \mura{\tt Box::new($v$)} moves the
    reference out of $v$, too, and it becomes the owning pointer
    \begin{columns}
      \begin{column}{0.6\textwidth}
  {\tt{\footnotesize
\begin{tabbing}
