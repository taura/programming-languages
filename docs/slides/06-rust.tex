...
%%%%%%%%%%%%%%%%% 
\section{Assignments move the value}
%%%%%%%%%%%%%%%%% 
%%%%%%%%%%%%%%%%% 
\begin{frame}
  \frametitle{Assignments do {\it not copy}, but {\it move}, the value}
  \begin{itemize}
  \item to maintain only one ``owner'' pointer,
    an assignment in Rust {\it moves} the value out of righthand side,
    disallowing further use of it
    \begin{center}
  {\tt{\footnotesize
\begin{tabbing}
012345\=aa\=aa\=\kill
...
\>\only<3->{\mura{// {\it the value moves away from a to b}}} \\
\>\only<3->{\mura{let b = a;}} \\
...
%%%%%%%%%%%%%%%%% 
\begin{frame}
  \frametitle{Argument-passing also moves the value}
  \begin{itemize}
  \item passing a value to a function also moves the value out
    \begin{columns}
      \begin{column}{0.6\textwidth}
  {\tt{\footnotesize
\begin{tabbing}
...
\>\only<2->{\mura{// {\it this also moves the value away from a}}} \\
\>\only<2->{\mura{f(a);}} \\
\>\only<2->{\aka{a.x}; // {\it NG, the value has moved out}} \\
\} \\
\end{tabbing}}}
\end{column}
      \begin{column}{0.4\textwidth}
...
%%%%%%%%%%%%%%%%% 
\begin{frame}
  \frametitle{Note: exceptions to ``assignment moves the value''}
  \begin{itemize}
  \item the value-moving assignment
  {\tt{\footnotesize
\begin{tabbing}
012345\=aa\=aa\=\kill
\>{\tt x = y;} \\
...
\item does it apply to a primitive type, say {\tt f64}?
  {\tt{\footnotesize
\begin{tabbing}
aa\=aa\=aa\=\kill
fn foo() \{ \\
\>let a = 123.456; \\
\>\mura{// {\it does the value move out from a!?}} \\
\>\mura{let b = a;} \\
...
\item and assignment or argument-passing of {\tt S}
  makes a copy of righthand side
...
%%%%%%%%%%%%%%%%% 
\section{{\tt Box<$T$>} (owning pointer) type}
%%%%%%%%%%%%%%%%% 
%%%%%%%%%%%%%%%%% 
\begin{frame}
  \frametitle{{\tt Box<$T$>} makes an owning pointer}
  \begin{itemize}
  \item making a pointer by \mura{\tt Box::new($v$)} moves the value
    out of $v$, too, and it becomes the owning pointer
    \begin{columns}
      \begin{column}{0.6\textwidth}
  {\tt{\footnotesize
\begin{tabbing}
...
\>a.x; // {\it OK, as expected} \\
\>a.y; // {\it OK, as expected} \\
\>\only<2->{\mura{\it // OK, now {\tt o} becomes the owning pointer}} \\
\>\only<2->{\mura{let o = Box::new(a)}} \\
\>\only<3->{\aka{a.x; // {\it NG, the value has moved out}}} \\
\>\only<4->{\ao{(*o).x;} // \ao{\it OK}} \\
\>\only<5->{\ao{o.x;} // \ao{\it OK. abbreviation of (*o).x}} \\
\} \\
\end{tabbing}}}
\end{column}
\begin{column}{0.4\textwidth}
\only<1>{\includegraphics[width=0.6\textwidth]{out/pdf/svg/move_assignment_1.pdf}}%
\only<2->{\includegraphics[width=0.6\textwidth]{out/pdf/svg/move_assignment_4.pdf}}
      \end{column}
    \end{columns}
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  % \item Rust implementation seems to allocate $T$ on stack and
  %   move it to heap when {\tt Box} is made, but again, it has nothing
  %   to do with lifetime (unlike C/C++)
  \end{itemize}
\end{frame}
...
%%%%%%%%%%%%%%%%% 
\section{{\tt \&$T$} (borrowing pointer) type}
%%%%%%%%%%%%%%%%% 
%%%%%%%%%%%%%%%%% 
\begin{frame}
