...
\begin{frame}
  ...
  \item in general,
    \begin{itemize}
    \item there may be so many intermediate results that some have to be spilled on memory
    \item values used after a function call must be saved on memory 
    \end{itemize}
...
\ifen
\begin{frame}[fragile]
  \frametitle{A simple code generation including function calls}
  \begin{itemize}
  \item []
\begin{lstlisting}
double integ(long n) {
