...
\fi
\ifen
\section{Implementing a compiler for a minimum C-like language}
\fi
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
\ifja
\begin{frame}
...
\begin{frame}
  ...
  \item<2-> \ao{Intermediate Representatin (IR) :}
    common representation portable across multiple source/target languages
  \end{itemize}

  \begin{center}
    \only<1>{\includegraphics[width=0.7\textwidth]{out/pdf/svg/inside_compiler_1.pdf}}%
    \only<2>{\includegraphics[width=0.7\textwidth]{out/pdf/svg/inside_compiler_2.pdf}}%
  \end{center}
\end{frame}
\fi
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
  \item a data structure that naturally represents a program
    \begin{itemize}
...
\begin{frame}
  ...
  \begin{itemize}
  \item \ao{lexer (lexical analyzer, tokenizer) :} string $\rightarrow$ sequence of ``tokens'' (words)
    \begin{itemize}
    \item[] ``{\tt while (x < 10) y++;}''
  ...
  \item \ao{parser :} sequence of tokens $\Rightarrow$ AST
...
\begin{frame}
  ...
  \item \ao{\it lexer generator} generates a lexer
    from the definition of \ao{\it tokens} (variables, numbers, \ldots)
  \item \ao{\it parser generator} generates a parser from
    the definition of higher-level constructs (expressions, statements, \ldots)
...
\begin{frame}
  ...
  \begin{itemize}
  \item a common representation of programs used by a compiler
  \item generally $\approx$ ``an assembly with unlimited variables''
  \item purposes
    \begin{enumerate}
    \item achieve portability
      \begin{itemize}
      ...
      \end{itemize}
    \item perform optimizations as IR $\rightarrow$ IR transformations
    \end{enumerate}
...
let ast_to_asm_program (Program(defs)) ... =
  ...
  header
  @ List.concat (List.map (fun def -> !\ao{\tt ast\_to\_asm\_def}! def ...) defs)
  @ trailer
\end{lstlisting}
    \end{column}
  \end{columns}
...
\begin{frame}[fragile]
  ...
  \begin{columns}
    ...
    \begin{column}{0.55\textwidth}
\begin{lstlisting}[escapechar=!]
let ast_to_asm_def def ... =
  match def with
    DefFun(f, params, ret_type, body) ->
      (gen_prologue def)
      @ (!\ao{\tt ast\_to\_asm\_stmt}! body ...)
      @ (gen_epilogue def)
\end{lstlisting}
    \end{column}
  \end{columns}
...
\begin{frame}[fragile]
  ...
  \begin{columns}
    ...
    \begin{column}{0.55\textwidth}
\begin{lstlisting}[escapechar=!]
let rec ast_to_asm_stmt stmt ...  =
  match stmt with
    ...
  | StmtWhile(cond, body) ->
    let cond_op,cond_insns = !\ao{\tt ast\_to\_asm\_expr}! cond ... in
    let body_insns = !\ao{\tt ast\_to\_asm\_stmt}! body ... in
    let ... in
    [ jmp Lc;
      Ls ]
    @ body_insns
...
\begin{lstlisting}[escapechar=!]
let rec ast_to_asm_expr expr ... =
  match expr with
    ...
  | ExprOp("+", [e0; e1]) ->
    let insns1,op1 = !\ao{\tt ast\_to\_asm\_expr}! e1 ... in
    let insns0,op0 = !\ao{\tt ast\_to\_asm\_expr}! e0 ... in
...
\begin{frame}[fragile]
  ...
  \begin{columns}
\begin{column}{0.45\textwidth}
\includegraphics[width=0.9\textwidth]{out/pdf/svg/translate_6.pdf}
\end{column}
\begin{column}{0.5\textwidth}
\begin{lstlisting}[escapechar=!]
let rec ast_to_asm_expr expr ... =
  match expr with
    ...
  | ExprCall(f, args) ->
     let insns,arg_vars = !\ao{\tt ast\_to\_asm\_exprs}! args env var_idx in
...
\begin{frame}[fragile]
  ...
  \begin{columns}
    \begin{column}{0.7\textwidth}
      \begin{itemize}
      \item {\tt ast\_to\_asm\_expr} $E$ receives a value ($v$)
        pointing to the lowest end of free space
      \item {\tt ast\_to\_asm\_expr} $E$ {\it v} \ldots\ 
        generates instructions that evaluate $E$ using
        (destroying) only addresses at or above $v${\tt (\%rsp)}
      \end{itemize}
    \end{column}
    \begin{column}{0.3\textwidth}
      \begin{center}
        \includegraphics[width=1.0\textwidth]{out/pdf/svg/stack_v.pdf}
      \end{center}
    \end{column}
  \end{columns}
\end{frame}
  ...
  \begin{columns}
    \begin{column}{0.34\textwidth}
      \begin{center}
        \includegraphics[width=0.9\textwidth]{out/pdf/lsvg/translate_4.pdf}
        \vskip5mm
        \includegraphics[width=0.5\textwidth]{out/pdf/svg/stack_v.pdf}
      \end{center}
    \end{column}
    \begin{column}{0.66\textwidth}
\begin{lstlisting}[escapechar=!]
let rec ast_to_asm_expr expr !\ao{$v$}! =
 match expr with
  ...
 | ExprOp("+", e0, e1) ->
  let insns1,op1 = !\ao{\tt ast\_to\_asm\_expr}! e1 !\ao{$v$}! .. in
  let insns0,op0 = !\ao{\tt ast\_to\_asm\_expr}! e0 (!\underline{\tt \ao{$v$} + 8}!) .. in
  let m = !\ao{$v$}!(%rsp) in
  ((insns1
  @ [ movq op1,m ]
  @ insns0
...
\item $\rightarrow$
  {\tt ast\_to\_asm\_stmt} also takes {\it v} pointing to the beginning of
  the free space
\item {\tt ast\_to\_asm\_stmt} $S$ $v$ \ldots\ generates
  instructions to execute $S$; they use (destroy)
  only addresses at or above $v${\tt (\%rsp)}
...
\begin{frame}[fragile]
  ...
  \item make a data structure that holds
    ...
    and pass it to {\tt ast\_to\_asm\_stmt} and {\tt ast\_to\_asm\_expr}
...
\ifen
\begin{frame}[fragile]
  \frametitle{{\tt ast\_to\_asm\_expr} receives an environment}
  \begin{itemize}
  \item []
\begin{lstlisting}[escapechar=!]
let rec ast_to_asm_expr expr !\ao{\it env}! !\ao{$v$}! =
  match expr with
    ...
  | ExprId(x) ->
...
\ifen
\begin{frame}[fragile]
  \frametitle{{\tt ast\_to\_asm\_stmt} receives an environment too}
  \begin{itemize}
  \item []
\begin{lstlisting}[escapechar=!]
let rec ast_to_asm_stmt stmt !\ao{\it env}! !\ao{$v$}! =
  match stmt with
    ...
...
\begin{frame}[fragile]
  ...
  \item {\it env', v'} {\tt = env\_extend} {\it decls} {\it loc} {\it env}
    \begin{itemize}
    \item [] can be easily built on {\tt env\_add} (left for you)
    \end{itemize}
